 ...
 * @LastEditTime: 2021-02-05 10:19:32
 */
...
export default class MyApiPreviewService extends React.Component {
	...
	async componentDidMount () {
		...
		this.setState({ detailLoading: true }, async () => {
			// TODO: userUuid需自动获取
			let result = await getInfo(this.props.initFieldUuid, { userUuid: 10002 })
			console.log(result)
			if (result) {
				if (result.code === 10000) {
					...
					this.field.setValue(
						'classificationUuids',
						result.result.classificationUuids
					)
					this.field.setValue(
						'classificationName',
						result.result.dmpCatalogueDirectoryVos[0]
							? result.result.dmpCatalogueDirectoryVos[0].name
							: ''
					)
					...
					this.field.setValue(
						'tokenExpireTime',
						moment(result.result.tokenExpireTime).format('YYYY-MM-DD')
					)
	...
	render () {
		...
		const switchSubscriptionStatus = (status) => {
			...
			switch (status) {
				case 0:
					label = '审核中'
					labelColor = colorStyle.status_progressing_color
					bgColor = colorStyle.status_bg_progressing_color
					break
				case 1:
					label = '已通过'
					labelColor = colorStyle.status_success_color
					bgColor = colorStyle.status_bg_success_color
					break
				case 2:
					label = '已驳回'
					labelColor = colorStyle.status_error_color
					bgColor = colorStyle.status_bg_error_color
					break
				default:
					break
			}
			...
			<Form labelAlign="top" field={this.field} style={{ width: '100%' }}>
				...
				<InfoContainer title="基本信息" id="basicInfo">
					...
					<FormItem label="URL：" required>
						<Input style={{ width: '100%' }} readOnly {...init('url')} />
					</FormItem>
				...
				<InfoContainer title="输入字段配置" id="inputParamSetting">
					<Table dataSource={this.field.getValue('apiInputTable')}>
						...
						/>
						<Table.Column
							dataIndex="fieldIsRequired"
							title="是否必填"
							align="center"
							width={50}
							cell={(value) => renderFieldIsRequired(value)}
						/>
						<Table.Column
				...
				<InfoContainer title="API请求样例" id="requestSample">
					<FormItem label="" required>
						...
						/>
						<CopyToClipboard
							text={this.field.getValue('requestUrl')}
							onCopy={() => Message.success('已复制该URL至粘贴板')}
						>
							<Button type="secondary" style={{ width: '80px' }}>
								复制URL
							</Button>
						</CopyToClipboard>
						<Button
							...
							<Button
								...
								onClick={() => {
									SubscriptionDialog.show({
										...
										onConfirm: async (v) => {
											this.onSubscription([initFieldUuid], v)
										},
									})
								}}
